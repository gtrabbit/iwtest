angular.module('iwApp', [])
	.controller('controller', ['$scope', function controller($scope){
		...
		$scope.filter = {
			...
			sortToggleChannels: false,
			sortToggleDevices: false,
			alphaToggle: false,
			typedFilter: null,
			selectedOption: null,
			updateFilter: (type, filter)=>{
				this.displayData = this.dataToDisplay(type, filter);
				this.updateTotals();
			}
		}


		// send this property to the view, initally the whole thing
		this.displayData = this.fullArray;

		//find total of currently displayed data
		this.calcTotals = function calcTotals(){
			let total = {
				'Devices': 0,
				'Channels': 0
			};
			for (let country of this.displayData){
				total['Devices'] += country.data['Devices'];
				total['Channels'] += country.data['Channels'];
			}
			return total;
		};


		$scope.totals = this.calcTotals();
		...
		// helper function determines which data to return
		this.dataToDisplay = function filterData(type, filter){
			switch(type){
				case 'byName':
					return [{
						name: filter,
				...
				case 'sort':
					let orderConf = filter.split(' - ');

					//cond for sorting by click, toggles between
					//ascending and descending
					if (orderConf.length === 1){
						let toggle;
						if (orderConf[0] === 'Channels'){
							toggle = $scope.filter.sortToggleChannels;
							$scope.filter.sortToggleChannels = !$scope.filter.sortToggleChannels;
						} else {
							toggle = $scope.filter.sortToggleDevices;
							$scope.filter.sortToggleDevices = !$scope.filter.sortToggleDevices
						}
						let appendment = toggle ? 'Ascending' : 'Descending';
						orderConf.push(appendment)
					}

					//sort ascending or descending
					if (orderConf[1] === 'Ascending'){
						return this.displayData.slice().sort((a, b)=>(
							a.data[orderConf[0]] - b.data[orderConf[0]]
							))
					} else {
						return this.displayData.slice().sort((a, b)=>(
							b.data[orderConf[0]] - a.data[orderConf[0]]
							))
					}
			
				// filtering from the input field
				case 'search':
					return this.fullArray.filter(a=>(
						a.name.toLowerCase().match(filter.toLowerCase())))		
					
				// sorting alphabetically from click
				case 'alpha':
						$scope.filter.alphaToggle = !$scope.filter.alphaToggle;
						return this.displayData.sort((a,b)=>{
							return a.name.toUpperCase() > b.name.toUpperCase()
								? result = $scope.filter.alphaToggle ? 1 : -1
								: result = $scope.filter.alphaToggle ? -1 : 1;
						})
					
				// the 'reset filters button' comes here	
				case 'clear':
				default:
					$scope.filter.typedFilter = null;
